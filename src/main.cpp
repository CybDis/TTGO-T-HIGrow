...
//           rel = "5.5.7"; // Fixed occasional "nan" MQTT payload (HA "non-finite value" error): formatSensorValue() now returns an empty sentinel for NAN readings (failed DHT/BME280 transaction), and updateHASensors() skips setValue()/mqttLog() for that sensor this cycle instead of publishing "nan", leaving the previously retained MQTT value in place
//           rel = "5.5.8"; // Fixed broken charging indicator: 5.5.6 added an upper clamp inside readBattery() that caps the returned value at 100, but main.cpp's charging detection relies on the mapped value exceeding 100 while the charger raises the voltage above the 4.2V full-charge point (checked as "bat > 120" before config.bat is separately clamped to 100 for display); with the value pre-clamped to 100 in readBattery(), that check could never fire again. Removed the upper clamp from readBattery(), keeping only the lower bound (bat < 0 -> 0)
const String rel = "5.5.9"; // Fixed Battery%/RawBattAdc flickering by several percent between wake cycles: readBattery() took a single unfiltered analogRead()/analogReadMilliVolts() sample, unlike Soil/Salt which use a 120-sample trimmed mean against the ESP32 ADC's known +-20..50 count noise; since the Battery% mapping only spans 0.9V (~1.1%/mV), that noise showed up as multi-percent jumps. readBattery() now uses the same trimmed-mean technique for both RawBattAdc and the calibrated voltage feeding Battery%
